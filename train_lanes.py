...
class BinaryMetrics:
    ...
    def update(self, preds, targets):
        # pairnoume th klash 1 (lane)
        preds = torch.argmax(preds, dim=1).cpu().numpy()
        targets = targets.cpu().numpy()
        
        # ypologizoume to IoU mono gia lorida (timh 1)
        pred_lane = (preds == 1)
...
class BDDLaneDataset(Dataset):
    def __init__(self, images_dir, masks_dir, transform=None):
        ...
        # filter: kratame mono tis eikones pou exoun mask
    ...
    def __getitem__(self, idx):
        ...
        if mask is None: 
            ...
            # metatrepoume to 255 (lefko) se 1 (klash lane)
            mask[mask == 255] = 1
...
# VISUALIZATION
# ==========================================
def save_predictions_as_images(loader, model, epoch, folder="training_results_lanes", device="cuda"):
    model.eval() 
    for x, y in loader:
        x = x.to(device)
        with torch.no_grad():
        ...
        # epeidh einai 0 kai 1, xrhsimopoioume cmap='gray' gia na fainetai san lefkh grammh sto fonto
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Μέσο Loss: {avg_loss:.4f} | Lane IoU: {iou_lane * 100:.2f}%")

        torch.save(model.state_dict(), os.path.join(MODELS_DIR, f"unet_lanes_epoch_{epoch+1}.pth"))
