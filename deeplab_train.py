...
def create_deeplab_model(num_classes):
    model = segm.deeplabv3_resnet50(pretrained=True, progress=True)
...
def train_one_epoch(model, loader, optimizer):
    ...
    for imgs, masks in tqdm(loader, desc="Train"):
        ...
        optimizer.zero_grad()
        out = model(imgs)["out"]   # DeepLab output
...
# ---------------------------
def save_sample_predictions(model, dataset, epoch, n=3):
    model.eval()
    os.makedirs(SAVE_DIR, exist_ok=True)
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # save checkpoint
        torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, f"deeplab_epoch{epoch}.pth"))
        if val_iou > best_iou:
            best_iou = val_iou
            torch.save(model.state_dict(), os.path.join(CHECKPOINT_DIR, "deeplab_best.pth"))
